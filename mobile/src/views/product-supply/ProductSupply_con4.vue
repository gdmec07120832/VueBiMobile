          ...
          <van-popup v-model="question_show" @close='showPopup_close' class="popup_css" get-container="body">
            ...
            <div style="text-align: left;font-size: 14px;padding-left: 20px;
              ...
              <van-collapse v-model="activeNames_tips" accordion>
                <van-collapse-item title="售后不良率" name="1">
                  <p>上月+当月产生售后责任/上月发货商品数量（如：2月考核，即取1月发货商品数作为分母，取（1+2月售后数作为分子）</p>
                  <p>①剔除采购经销（不剔除网拍订单）；剔除床垫系列、排骨架系列、气动杆系列、赠品系列和某些特定商品</p>
                  <p>②责任类型为工厂责任+原材料责任+物流三包（损坏、装错装坏）</p>
                </van-collapse-item>

                

                <van-collapse-item title="工厂汇总不良率" name="2">
                  <p>上月+当月产生售后责任/上月发货商品数量（如：2月考核，即取1月发货商品数作为分母，取（1+2月售后数作为分子）</p>
                  <p>①剔除采购经销（不剔除网拍订单）；剔除床垫系列、排骨架系列、气动杆系列、赠品系列和某些特定商品</p>
                  <p>②责任类型为工厂责任+原材料责任</p>
                </van-collapse-item>
                ...
                <van-collapse-item title="物流三包不良率" name="3">
                  <p>上月+当月产生售后责任/上月发货商品数量（如：2月考核，即取1月发货商品数作为分母，取（1+2月售后数作为分子）</p>
                  <p>①剔除采购经销（不剔除网拍订单）；剔除床垫系列、排骨架系列、气动杆系列、赠品系列和某些特定商品；</p>
                  <p>②责任类型为物流三包（损坏、装错装坏）。</p>
                </van-collapse-item>
              </van-collapse>
            ...
            >
              ...
              <span :style="sum_zhibiao.body">
                ...
                <i style="font-style: normal;"> 
                  ...
                  (sel_value.Range_on.Range_set=='4')?
                  nll_undefined((sel_title_setvalue.quota_value.cont2*100).toFixed(2))+'%': 
                  nll_undefined((sel_title_setvalue.quota_value.cont2/10000).toFixed(1))+'万'}} 
              ...
              <span class="tongbi_css" :style="sum_zhibiao.body">
                ...
                style="font-style: normal;padding-left: 3px;"
                  ...
                  :class="
                      ...
                      (sel_value.Range_on.Range_set=='4'&&sel_title_setvalue.quota_value.cont3==0)?
                      ''
                      :
                        Number(
                            sel_title_setvalue.quota_value.cont3
                ...
                >
                  {{
                    ...
                    (sel_value.Range_on.Range_set=='4'&&sel_title_setvalue.quota_value.cont3==0)?
                      '--'
                    : nll_undefined((sel_title_setvalue.quota_value.cont3*100).toFixed(2))+'%'
            ...
            >
              ...
              <span :style="sum_zhibiao.body">
                ...
                <i style="font-style: normal"> {{(sel_title_setvalue.quota_value.cont5==0)? '--':
                  ...
                  (sel_value.Range_on.Range_set=='4')? 
                  nll_undefined((sel_title_setvalue.quota_value.cont5*100).toFixed(2))+'%': 
              ...
              <span class="tongbi_css" :style="sum_zhibiao.body">
                ...
                >
                  {{
                    ...
                    '--'
                    :
                    nll_undefined((sel_title_setvalue.quota_value.cont6*100).toFixed(2))+'%'
                  }}</i
                >
              </span>
            </div>
              ...
              <span class="tongbi_css" :style="sum_zhibiao.body">
                ...
                :style="(sel_value.Range_on.Range_set=='4'&&sel_title_setvalue.quota_value.cont9==0)?{'padding-left': '0px'}:{'padding-left': '3px'}"
                  ...
                  >{{
                    ...
                    ' --'
                    :
                    nll_undefined((sel_title_setvalue.quota_value.cont9*100).toFixed(2))+'%'
...
export default {
  ...
  created() {
    ...
    const apiMap = {
      ...
      // "SQL-pds_afs-tgt_afs_bad_rto": { resultProp: "tgt_afs_bad_rto" }, //没用上的了
      // "SQL-pds_afs-afs_bad_rto_yoy": { resultProp: "afs_bad_rto_yoy" }, //没用上的了
      // "SQL-pds_afs-factory_bad_rto": { resultProp: "factory_bad_rto" }, //没用上的了
      // "SQL-pds_afs-afs_bad_rto": { resultProp: "afs_bad_rto" },  //没用上的了
      // "SQL-pds_afs-lgs_bad_rto": { resultProp: "lgs_bad_rto" }, //没用上的了

      

      // "SQL-pds_afs-mons_afs_rto": { resultProp: "mons_afs_rto" },
      // "SQL-pds_afs-spl_afs_rto": { resultProp: "spl_afs_rto" },
      "SQL-pds_afs-pds_afs_tot": { type: Array, resolve4: resolve4 },
      "SQL-pds_afs-mons_afs_rto": { type: Array, resolve3: resolve3 },
      "SQL-pds_afs-spl_afs_rto": { type: Array, resolve2: resolve2 },
      ...
      //库存
      // "SQL-pds_inv-spl_inv_amt": { resultProp: "spl_inv_amt" },
      // "SQL-pds_inv-rem_inv_lvl": { resultProp: "rem_inv_lvl" },
      // "SQL-pds_inv-intrns_amt": { resultProp: "intrns_amt" },
      // "SQL-pds_inv-tot_inv_amt": { resultProp: "tot_inv_amt" },
      // "SQL-pds_inv-tot_inv_amt_yoy": { resultProp: "tot_inv_amt_yoy" },
      // "SQL-pds_inv-max_tot_inv_amt": { resultProp: "max_tot_inv_amt" },
      // "SQL-pds_inv-diff_tot_inv_amt": { resultProp: "diff_tot_inv_amt" },
      // "SQL-pds_inv-linsy_inv_amt": { resultProp: "linsy_inv_amt" },
  ...
  mounted() {
    ...
    this.$EventBus.$on("prosupply_choice_f", (Range_value_set) => {
      ...
      switch (Range_value_set.Range_value_set) {
        ...
        case "4":
          ...
          Information_log('产品供应-品质','产品供应-品质:  '+'数据加载完成');
    ...
    this.sel_value.Range_on.Range_set = '4';
    this.Single_choice('cpcy', ['_this.change_text(\''+'4'+'\')'], true, '_this.sel_value.theme');
    ...
    change_text(value) {
      switch (value) {
        ...
        case "4":            
          ...
          this.sel_title.chanzhi_wenzi.title_muban = "工厂汇总不良率";
      ...
      this.p1.then(() => {
        ...
        if(this.sel_value.Range_on.Range_set=='1'){
          ...
          try{
            ...
            // this.sel_title_setvalue.quota_value.cont5=0;
            this.sel_title_setvalue.quota_value.cont5=data.WG_TGT;
            // this.sel_title_setvalue.quota_value.cont6=data.FAC_AFS_RTO_YOY;
            this.sel_title_setvalue.quota_value.cont6=data.AFS_RTO_DIFF;
      ...
      this.p1.then(() => {
        ...
        try{
          ...
          if(this.sel_value.Range_on.Range_set == "1"){
            ...
            // }
            // 做M+1处理
            // console.log(_time_now_M.getMonth());
            ...
            Object.keys(user_data).forEach((key_value_chart) => {
              if(this.sel_value.target.all_muri_value==1){
                ...
                if(user_data[key_value_chart].MDATE.substring(0,4)*1==year_now){
                  sale_amt.push((user_data[key_value_chart].AFS_RTO*100).toFixed(2)*1);
                }else if(user_data[key_value_chart].MDATE.substring(0,4)*1==(year_now-1)){
                  ago_amt.push((user_data[key_value_chart].AFS_RTO*100).toFixed(2)*1);
                  hour_data.push((user_data[key_value_chart].MDATE.substring(5, 7)*1+'月'));
                }
                ...
                if(user_data[key_value_chart].MDATE.substring(0,4)*1==year_now){
                  sale_amt.push((user_data[key_value_chart].FAC_AFS_RTO*100).toFixed(2)*1);
                }else if(user_data[key_value_chart].MDATE.substring(0,4)*1==(year_now-1)){
                  ago_amt.push((user_data[key_value_chart].FAC_AFS_RTO*100).toFixed(2)*1);
                  hour_data.push((user_data[key_value_chart].MDATE.substring(5, 7)*1+'月'));
                }
                ...
                if(user_data[key_value_chart].MDATE.substring(0,4)*1==year_now){
                  sale_amt.push((user_data[key_value_chart].LGS_AFS_RTO*100).toFixed(2)*1);
                }else if(user_data[key_value_chart].MDATE.substring(0,4)*1==(year_now-1)){
                  ago_amt.push((user_data[key_value_chart].LGS_AFS_RTO*100).toFixed(2)*1);
                  hour_data.push((user_data[key_value_chart].MDATE.substring(5, 7)*1+'月'));
                }
          ...
          if(this.sel_value.Range_on.Range_set == "1"){
            ...
            Object.keys(hour_data).forEach((key_mv) => {
              // if(hour_data[key_mv]==((_time_now*1)+'月')){
              if(hour_data[key_mv]==((_time_now*1)+'月')){
                this.barData.value=(sale_amt[key_mv])?(this.fomatMoney((sale_amt[key_mv].toFixed(2)))+'%'):'--'
                this.barData.ago_value=(ago_amt[key_mv])?this.fomatMoney(ago_amt[key_mv].toFixed(2))+'%':'--'
              }
            });  

            if(this.barData.value==0&&this.barData.ago_value==0){
              this.barData.month=year_now+'.'+(hour_data[hour_data.length-1]).replace(/[^0-9]/ig,"").substring(0,2);
              this.barData.value=this.fomatMoney(sale_amt[sale_amt.length-1].toFixed(2))+'%'
              this.barData.ago_value=this.fomatMoney(ago_amt[sale_amt.length-1].toFixed(2))+'%'
            }          
              ...
              tooltip: {
                ...
                formatter:function (params) {
                  ...
                  if(_this.sel_value.Range_on.Range_set=='1'){
                    ...
                    _this.barData.value =(params[1]==undefined)? '--':_this.fomatMoney((params[1].value).toFixed(2))+'%';
                    _this.barData.ago_value =(params[0]==undefined)? '--': _this.fomatMoney((params[0].value).toFixed(2))+'%';
                    // _this.barData.month =(params[0]==undefined)? 0: params[0].axisValue;
                    _this.barData.month =(params[0]==undefined)? 0: year_now+'.'+((params[0].axisValue).replace(/[^0-9]/ig,"")>9?(params[0].axisValue).replace(/[^0-9]/ig,""):'0'+(params[0].axisValue).replace(/[^0-9]/ig,""));
          ...
          console.error(error);
          // this.info_error+=1;
          Information_log('产品供应-品质','产品供应-品质-折线图:  '+error.toString());
        }
