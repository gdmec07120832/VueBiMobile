          ...
          <van-popup v-model="question_show" @close='showPopup_close' class="popup_css" get-container="body">
            ...
            <div style="text-align: left;font-size: 14px;padding-left: 20px;
              ...
              <p style="font-weight: bold;">
                ...
                <p style="margin-bottom: 0px;margin-top: 5px;">T+1(数据截止到昨天)</p> 
                <p style="margin-bottom: 5px;margin-top: 5px;">时间范围：当月（当月1号-昨天）</p> 


              <p style="font-weight: bold;">
                  <i style="width: 5px;height: 16px;background-color: #23B0A8;display: block;float: left;margin-right: 5px;margin-top: 3px;border-radius: 4px;"></i>
                  <span style="color: #000000;">新品定义</span></p>
                <p style="margin-bottom: 0px;margin-top: 5px;">统计年份上架的单品升级性质=原始产品的sku</p>
              ...
              <van-collapse v-model="activeNames_tips" accordion>
                <van-collapse-item title="新品业绩" name="1">
                  <p>统计时间为支付时间的新品支付业绩</p>
                </van-collapse-item>

                <van-collapse-item title="新品业绩占比" name="2">
                  <p>新品支付业绩/总支付业绩</p> 
                </van-collapse-item>

                <van-collapse-item title="上架sku数" name="3">  
                  <p>时间范围内新上架的sku数目</p>
                </van-collapse-item>
              </van-collapse>
          ...
          >
            ...
            >
              ...
              <span :style="sum_zhibiao.body">
                <i
                  style="width: 35%; display: inline-block; font-style: normal"
              ...
              <span class="tongbi_css" :style="sum_zhibiao.body">
                <i
                  ...
                  "
                  >
                  {{ sel_title.chanzhi_wenzi.reach_rate }}
                  </i
                >
                ...
                <!--  sel_value.target.all_muri_value == 1?
                      ...
                      : -->
                <i 
                style="font-style: normal;padding-left: 3px;" :style="{color:(sel_title_setvalue.quota_value.cont3>1)?'#ff4d4f':'#008000'}">
                  {{
            ...
            >
              ...
              <span :style="sum_zhibiao.zhognxin"> 
                ...
                {{(sel_title_setvalue.quota_value.cont4*100).toFixed(1)+'%'}} </span>


              <span :style="sum_zhibiao.body">
                <i
                  style="width: 50%; display: inline-block; font-style: normal"
                  >{{ sel_title.chanzhi_wenzi.ago_amt }}</i
                >

                <i style="font-style: normal"> {{(sel_title_setvalue.quota_value.cont5==0)? '--':
                  (sel_title_setvalue.quota_value.cont5*100).toFixed(1)+'%'}} 
                </i>
              </span>
              <span class="tongbi_css" :style="sum_zhibiao.body">
                <i
            ...
            >
              ...
              <span :style="sum_zhibiao.zhognxin"> 
                {{fomatMoney((sel_title_setvalue.quota_value.cont7*1).toFixed(0))+'个'}} </span>
              ...
              <span :style="sum_zhibiao.body">
                ...
                <i style="font-style: normal"> {{(sel_title_setvalue.quota_value.cont8==0)? '--':
                  fomatMoney((sel_title_setvalue.quota_value.cont8*1).toFixed(0))+'个'}} 
                </i>
              </span>
              <span class="tongbi_css" :style="sum_zhibiao.body">
                <i
          ...
          <section class="px10 py10" :style="(sel_value.Range_on.Range_set=='2'||sel_value.Range_on.Range_set=='5')? echarts2_css_n : echarts2_css_y">
            <ul class="table_all">
              <li class="table_title">
                <ul>
                  <li>项目组</li>
                  <li>{{(sel_value.Range_on.Range_set=='1')?'本期':'业绩（万）'}}</li>
                  <li>{{(sel_value.Range_on.Range_set=='1')? '同期':'业绩达成'}}</li>
                  <li>{{(sel_value.Range_on.Range_set=='4')? '差额':(sel_value.Range_on.Range_set=='1')? '同比':'业绩占大盘'}}</li>
                  <li>上架sku数</li>
                </ul>
              </li>
              ...
              >
                ...
                <ul>
                  ...
                  <li>{{nll_undefined(fomatMoney(((val.NEW_SALE_AMT||0)/10000).toFixed(1)))+'万'}}</li>
                  <li>{{nll_undefined(fomatMoney(((val.NEW_MTRL_CMPL_RTO||0)*100).toFixed(1)))+'%'}}</li>

                  <!--:class="(
                      (sel_value.Range_on.Range_set=='4')? 
                      Number(val.NEW_MTRL_TOT_AMT_RTO||0)>=0?'rate-up_salesR':'rate-down_salesR':
                      (sel_value.Range_on.Range_set=='1')? 
                      Number(val.NEW_MTRL_TOT_AMT_RTO||0)>=0?'rate-up_sales':'rate-down_sales'
                      :Number(val.NEW_MTRL_TOT_AMT_RTO||0)>=1?'rate-up_sales':'rate-down_sales')" -->
                  <li>
                    {{nll_undefined(((val.NEW_MTRL_TOT_AMT_RTO||0)*100).toFixed(1))+'%'}}
                  </li>

                  <li>{{nll_undefined(fomatMoney((val.SKU_CNT||0).toFixed(0)))+'个'}}</li>  
...
export default {
  ...
  created() {
    ...
    const apiMap = {
      "SQL-pds_new_prod-mtd_sum": { type: Array, resolve4: resolve4 },
      "SQL-pds_kpi-kpi_fields": { type: Array, resolve3: resolve3 },  //更新时间来的
      "SQL-pds_new_prod-mly_trend": { type: Array, resolve2: resolve2 },
      "SQL-pds_new_prod-spl_cmpl": { type: Array, resolve: resolve1 },
      // 新加的 新品改未sku
    };
  ...
  mounted() {
    ...
    this.$EventBus.$on("prosupply_choice_f", (Range_value_set) => {
      ...
      // console.log("========aaaaaaaaaaaaasda ad a           asd==============");
    ...
    this.sel_value.Range_on.Range_set = '6';
    this.Single_choice('cpcy', ['_this.change_text(\''+'6'+'\')'], true, '_this.sel_value.theme');

    console.log("==================啊实打实的  阿斯顿1   阿斯顿1 ==========================");
  ...
  methods: {
    ...
    change_text(value) {
      switch (value) {
        ...
        case "6":
          ...
          this.sel_title.chanzhi_wenzi.title_all = "新品业绩";
          this.sel_title.chanzhi_wenzi.title_muban = "新品业绩占比";
          this.sel_title.chanzhi_wenzi.title_ruanti = "上架sku数";
          this.sel_title.chanzhi_wenzi.target = "目标";
          this.sel_title.chanzhi_wenzi.reach_rate = "达成";
          this.sel_title.chanzhi_wenzi.ago_rate = "同比";
          this.sel_title.chanzhi_wenzi.ago_amt = "同期";
          break;
    ...
    data_screen_quota(){
      ...
      this.p1.then(() => {
        ...
        }else if(this.sel_value.Range_on.Range_set=='6'){


          try{

            // console.log("++++++++++++新品的开始！++++++++++++++++");
            data=this.data["pds_new_prod-mtd_sum"][0];
            // console.log(data);  


            this.sel_title_setvalue.quota_value.cont1=data.NEW_SALE_AMT;
            // this.sel_title_setvalue.quota_value.cont2=data.TOT_INV_AMT_DIFF;
            this.sel_title_setvalue.quota_value.cont2=data.NEW_MTRL_TGT;
            this.sel_title_setvalue.quota_value.cont3=data.NEW_MTRL_CMPL_RTO;

            this.sel_title_setvalue.quota_value.cont4=data.NEW_MTRL_TOT_AMT_RTO;
            // this.sel_title_setvalue.quota_value.cont5=data.LINSY_INV_AMT_DIFF;
            this.sel_title_setvalue.quota_value.cont5=data.AGO_NEW_MTRL_TOT_AMT_RTO;
            this.sel_title_setvalue.quota_value.cont6=data.DIFF_NEW_MTRL_TOT_AMT_RTO;


            this.sel_title_setvalue.quota_value.cont7=data.SKU_CNT;
            // this.sel_title_setvalue.quota_value.cont8=data.SPL_INV_AMT_DIFF;
            this.sel_title_setvalue.quota_value.cont8=data.AGO_SKU_CNT;
            this.sel_title_setvalue.quota_value.cont9=data.NEW_MTRL_SKU_CNT_YOY;
            // console.log(this.sel_title_setvalue.quota_value.cont1);

            data_project=this.data["pds_new_prod-spl_cmpl"]; 
            data_project = data_project.sort((a, b) => {
              let value_sort = "";
              let a_time = a["NEW_SALE_AMT"];
              let b_time = b["NEW_SALE_AMT"];
              value_sort =  Number(b_time) - Number(a_time);
              return value_sort;
            });
            // console.log(data_project);
            ...
            console.error(error);
            // this.info_error+=1;
            Information_log('产品供应-新品','产品供应-新品-核心指标:  '+error.toString());
          } 
      ...
      this.p1.then(() => {
        ...
        try{
          ...
          if(this.sel_value.Range_on.Range_set == "1"){
            ...
            user_data = this.data["pds_new_prod-mly_trend"];
            user_data = user_data.sort((a, b) => {
              let value_sort = "";
              let a_time = new Date(a["MDATE"]).getTime(new Date(a["MDATE"]));
              let b_time = new Date(b["MDATE"]).getTime(new Date(b["MDATE"]));
            ...
            Object.keys(user_data).forEach((key_value_chart) => {
              ...
              // ,NEW_SALE_AMT              -- 新品业绩
              // ,AGO_NEW_SALE_AMT          -- 同期新品业绩

              if(this.sel_value.target.all_muri_value==1&&user_data[key_value_chart].MDATE.substring(0,4)==(year_now-1)+''){
                hour_data.push((user_data[key_value_chart].MDATE.substring(5, 7)*1+'月'));
                ago_amt.push((user_data[key_value_chart].NEW_SALE_AMT/10000).toFixed(1)*1);
              }else if(this.sel_value.target.all_muri_value==2&&user_data[key_value_chart].MDATE.substring(0,4)==(year_now-1)+''){
                hour_data.push((user_data[key_value_chart].MDATE.substring(5, 7)*1+'月'));
                ago_amt.push((user_data[key_value_chart].NEW_MTRL_TOT_AMT_RTO*100).toFixed(1)*1);                
              }else if(this.sel_value.target.all_muri_value==3&&user_data[key_value_chart].MDATE.substring(0,4)==(year_now-1)+''){
                hour_data.push((user_data[key_value_chart].MDATE.substring(5, 7)*1+'月'));
                ago_amt.push((user_data[key_value_chart].SKU_CNT).toFixed(0)*1); 
              }

              if(this.sel_value.target.all_muri_value==1&&user_data[key_value_chart].MDATE.substring(0,4)==year_now){
                sale_amt.push((user_data[key_value_chart].NEW_SALE_AMT/10000).toFixed(1)*1);
              }else if(this.sel_value.target.all_muri_value==2&&user_data[key_value_chart].MDATE.substring(0,4)==year_now){
                sale_amt.push((user_data[key_value_chart].NEW_MTRL_TOT_AMT_RTO*100).toFixed(1)*1);
              }else if(this.sel_value.target.all_muri_value==3&&user_data[key_value_chart].MDATE.substring(0,4)==year_now){
                sale_amt.push((user_data[key_value_chart].SKU_CNT).toFixed(0)*1);
              }
            ...
            Object.keys(hour_data).forEach((key_mv) => {
              if(hour_data[key_mv]==((_time_now.getMonth()+1)+'月')){
                this.barData.value=this.fomatMoney(sale_amt[key_mv])+
                  ((this.sel_value.target.all_muri_value==1)?'万':(this.sel_value.target.all_muri_value==2)?'%':'个');
                ...
                formatter:function (params) {
                  ...
                  if(_this.sel_value.Range_on.Range_set=='1'){
                    ...
                    _this.barData.value =(params[1]==undefined)? '--':_this.fomatMoney((params[1].value).toFixed(1))+
                      ((_this.sel_value.target.all_muri_value==1)?'万':(_this.sel_value.target.all_muri_value==2)?'%':'个');
