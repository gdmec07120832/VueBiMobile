          ...
          <van-popup v-model="question_show" @close='showPopup_close' class="popup_css" get-container="body">
            ...
            <div style="text-align: left;font-size: 14px;padding-left: 20px;
              ...
              <van-collapse v-model="activeNames_tips" accordion>
                <van-collapse-item title="日均订单欠货（销）" name="1">
                  <p>当月日订单欠货汇总（可用数-到期订单数）/当月欠货日天数</p>
                  <p>订单欠货金额：以剔除停产产品的到期订单数为基础，用对应订单仓库库存可用数进行分配，未能满足的订单视为欠货订单，该类订单金额总和为订单欠货金额。</p>
                </van-collapse-item>
                <van-collapse-item title="日均商品欠货（采）" name="2">
                  <p>当月日商品欠货汇总（可用数-到期订单数）/当月欠货日天数</p>
                  <p>商品欠货采购价：以剔除停产产品的到期订单数为基础，用对应订单仓库库存可用数进行分配，未能满足的商品为欠货数量，欠货数量*平均采购价为商品欠货采购额。</p>
                </van-collapse-item>
                <van-collapse-item title="当日商品欠货（采）" name="3">
                  以剔除停产产品的到期订单数为基础，用对应订单仓库库存可用数分配，未能满足的商品为欠货数量，欠货数量*平均采购价为商品欠货采购额。
                </van-collapse-item>
              </van-collapse>
...
export default {
  ...
  created() {
    ...
    const apiMap = {
      ...
      "SQL-pds_shrt-shrt_days": { type: Array, resolve3: resolve3 },
      "SQL-pds_kpi-kpi_fields": { type: Array, resolve2: resolve2 },
  ...
  mounted() {
    ...
    this.Single_choice('cpcy', ['_this.change_text(\''+'3'+'\')'], true, '_this.sel_value.theme');
  ...
  methods: {
    ...
    onRefresh() {
      ...
      let apiMap3 = {
        ...
        "SQL-pds_shrt-shrt_days": { resultProp: "shrt_days" },
        "SQL-pds_kpi-kpi_fields": { resultProp: "kpi_fields" },
        "SQL-pds_shrt-shrt_spl": { resultProp: "shrt_spl" },
      };
    ...
    change_text(value) {
      switch (value) {
        ...
        case "3":            
          ...
          this.sel_title.chanzhi_wenzi.title_all = "日均订单欠货(销)";
          this.sel_title.chanzhi_wenzi.title_muban = "日均商品欠货(采)";
          this.sel_title.chanzhi_wenzi.title_ruanti = "昨天商品欠货(采)";
      ...
      this.p1.then(() => {
        ...
        if(this.sel_value.Range_on.Range_set=='1'){
          ...
          try{
            ...
            this.sel_title_setvalue.quota_value.cont5=data.TARGET_EXPURPRICE;
            ...
            this.sel_title_setvalue.quota_value.cont8=data.TGT_REMAIN_PPRICE;
    ...
    initChart() {
      ...
      this.p1.then(() => {
        ...
        try{
          ...
          console.log('指标：       '+this.sel_value.target.all_muri_value);
          console.log('主题：       '+this.sel_value.Range_on.Range_set);
          ...
          if(this.sel_value.Range_on.Range_set == "1"){
            ...
            let month_setup=(((_time_now.getMonth()+1)>9)?(_time_now.getMonth()+1):'0'+(_time_now.getMonth()+1));
            let day_setup=((_time_now.getDate()*1)>9?(_time_now.getDate()*1):'0'+(_time_now.getDate()*1));

            this.barData.month=year_now+'.'+month_setup+'.'+day_setup;


            Object.keys(hour_data).forEach((key_mv) => {
              if(hour_data[key_mv]==(month_setup+'-'+day_setup)){
                this.barData.value=this.fomatMoney(sale_amt[key_mv].toFixed(1))+'万'
                this.barData.ago_value=this.fomatMoney(ago_amt[key_mv].toFixed(1))+'万'
              }
            });  


            // console.log("欠货的！！！！！！！！！！！！");
            // console.log(hour_data[hour_data.length-1].replace(/[^0-9]/ig,"").substring(2,4));
            let substring_val=hour_data[hour_data.length-1].replace(/[^0-9]/ig,"");
            if(this.barData.value==0&&this.barData.ago_value==0){
              this.barData.month=year_now+'.'+substring_val.substring(0,2)+'.'+substring_val.substring(2,4);
              this.barData.value=this.fomatMoney(sale_amt[sale_amt.length-1].toFixed(1))+'万'
              this.barData.ago_value=this.fomatMoney(ago_amt[sale_amt.length-1].toFixed(1))+'万'
            }
              ...
              tooltip: {
                ...
                formatter:function (params) {
                  ...
                  if(_this.sel_value.Range_on.Range_set=='1'){
                    ...
                    let substring_val_s=params[0].axisValue.replace(/[^0-9]/ig,"");

                    _this.barData.value =(params[1]==undefined)? '--':_this.fomatMoney((params[1].value).toFixed(1))+'万';
                    _this.barData.ago_value =(params[0]==undefined)? '--': _this.fomatMoney((params[0].value).toFixed(1))+'万';
                    _this.barData.month =(params[0]==undefined)? 0: (_this.unique(year_date)).length==2&&(params[0].axisValue.substring(0,2)=='12')? 
                    (year_now-1)+'.'+substring_val_s.substring(0,2)+'.'+substring_val_s.substring(2,4)
                    :year_now+'.'+substring_val_s.substring(0,2)+'.'+substring_val_s.substring(2,4);
