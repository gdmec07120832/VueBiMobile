   ...
   <van-pull-refresh v-model="isLoading" :head-height="0" :pull-distance="0" @refresh="onRefresh">
    ...
    <div>
          ...
          <van-popup v-model="question_show" @close='showPopup_close' class="popup_css" get-container="body">
            ...
            <div style="text-align: left;font-size: 14px;padding-left: 20px;
              ...
              <van-collapse v-model="activeNames_tips" accordion>
                ...
                <van-collapse-item title="毛利率" name="3">
                  <p>采购毛利率=采购毛利额/主营业务收入*100%</p>
                  <p>①采购毛利额=主营业务收入-商品发货成本</p>
      ...
      <div class="canvas_bottom" ref="canvas_w_h" style="padding-top: 10px;background: #ffffff;margin-left: 10px;margin-right: 10px;border-radius: 4px;padding-bottom: 10px;">
        ...
        <van-empty description="母婴毛利率暂无二级类目" v-if="sel_value.canvas_no==true&&(sel_value.all_muri_value == 3 && sel_value.Range_set == 3)" />
...
export default {
  ...
  created() {
    ...
    //   // "SQL-ouk_qc-all_ouk_catgy_qc_rate": {
    //   //   resultProp: "ALL_OUK_CATGY_QC_RATE",
    //   // },
    //   // "SQL-ouk_qc-banmu_ouk_catgy_qc_rate": {
    //   //   resultProp: "BANMU_OUK_CATGY_QC_RATE",
    //   // },
    //   // "SQL-ouk_qc-ruant_ouk_catgy_qc_rate": {
    //   //   resultProp: "RUANT_OUK_CATGY_QC_RATE",
    //   // },
    ...
    const apiMap2 = {
      ...
      "SQL-muying_overview-catgy_pay_m": { resultProp: "CATGY_PAY_M" },
    ...
    initChart() {
      ...
      this.p1.then(() => {
        ...
        try{
          ...
          this.set_time_range.start = (month_yong[0]).substring(0,2)+'.'
          +(month_yong[0]).substring(3,5)

          // this.set_time_range.end = month_yong[month_yong.length - 1];
          this.set_time_range.end = (month_yong[month_yong.length - 1]).substring(0,2)+'.'
          +(month_yong[month_yong.length - 1]).substring(3,5)
          ...
          if(this.sel_value.day_7 == false && this.sel_value.day_30 == false){
            ...
            let set_on_time_now=(this.sel_value.Range_set==1||this.sel_value.Range_set==2)?(new Date(month_m1)):_time_now;
            ...
            this.barData.month=year_now+'.'+
            ((month_yong[month_yong.length-1].substring(0,2)+'.'+month_yong[month_yong.length-1].substring(3,5))||0);

            this.barData.value=this.fomatMoney((value_use[value_use.length-1]*1||0).toFixed(1))+(this.sel_value.Range_set==2||this.sel_value.all_muri_value==3?'%':'万');
            this.barData.ago_value=this.fomatMoney((ago_value[ago_value.length-1]*1||0).toFixed(1))+(this.sel_value.Range_set==2||this.sel_value.all_muri_value==3?'%':'万');
            ...
            Object.keys(month_yong).forEach((key_mv) => {
              if(month_yong[key_mv]==(_time_now_set)){
                this.barData.value=this.fomatMoney((value_use[key_mv]*1).toFixed(1)||'--')+(this.sel_value.Range_set==2||this.sel_value.all_muri_value==3?'%':'万');
                this.barData.ago_value=this.fomatMoney((ago_value[key_mv]*1||0).toFixed(1))+(this.sel_value.Range_set==2||this.sel_value.all_muri_value==3?'%':'万');
              }
            });  
          ...
          if(this.sel_value.day_7 == true || this.sel_value.day_30 == true){
            Object.keys(month_yong).forEach((key_user) => {
              month_yong[key_user]=(month_yong[key_user]).substring(0,2)+'.'+(month_yong[key_user]).substring(3,5)
            });
          }
              ...
              formatter: function (params) {
                ...
                if(_this.sel_value.day_7 == true || _this.sel_value.day_30 == true){
                  ...
                  _this.barData.value = (params[1]==undefined||params[1].value==undefined)?'--':
                    (_this.fomatMoney((params[1].value*1).toFixed(1))+(_this.sel_value.Range_set==2||_this.sel_value.all_muri_value==3?'%':'万'));
                  _this.barData.ago_value = (params[0]==undefined||params[0].value==undefined)?'--':
                    (_this.fomatMoney((params[0].value*1).toFixed(1))+(_this.sel_value.Range_set==2||_this.sel_value.all_muri_value==3?'%':'万'));
                  _this.barData.month =((month_year.length==2&&params[0].axisValue.substring(0,2)=='12')?year_now-1:year_now)+'.'+
                  ((params[0].axisValue==undefined)?0:(params[0].axisValue).substring(0,2)+'.'+(params[0].axisValue).substring(3,5));
                
                }else{


                  if(params.length==1){
                    _this.barData.value = (params[0]==undefined||params[0].value==undefined)?'--':
                      (_this.fomatMoney((params[0].value*1).toFixed(1))+(_this.sel_value.Range_set==2||_this.sel_value.all_muri_value==3?'%':'万'));
                  }else{
                    _this.barData.value = (params[1]==undefined||params[1].value==undefined)?'--':
                      (_this.fomatMoney((params[1].value*1).toFixed(1))+(_this.sel_value.Range_set==2||_this.sel_value.all_muri_value==3?'%':'万'));
