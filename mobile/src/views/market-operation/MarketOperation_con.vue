        ...
        <van-popup v-model="question_show" @close='showPopup_close' class="popup_css" get-container="body">
          ...
          <div style="text-align: left;font-size: 14px;padding-left: 20px;
            ...
            <p style="font-weight: bold;">
              ...
              <p style="margin-bottom: 20px;margin-top: 5px;">预计每天10:30更新昨日数据</p>
            ...
            <!-- <p style="font-weight: bold;">
              <i style="width: 5px;height: 16px;background-color: #23B0A8;display: block;float: left;margin-right: 5px;margin-top: 3px;border-radius: 4px;"></i>
              <span style="color: #000000;">商品访客</span>
            </p>
            <p>
              统计周期内浏览商品微详情3秒及以上、访问宝贝详情页的去重人数，一个人在统计时间范围内访问多次只记为一个。所有终端商品访客数为PC端商品访客数和无线端商品访客数相加去重。
            </p> -->
          ...
          <div style="text-align: left;font-size: 14px;padding-left: 20px;
            ...
            <p style="font-weight: bold;">
              <i style="width: 5px;height: 16px;background-color: #23B0A8;display: block;float: left;margin-right: 5px;margin-top: 3px;border-radius: 4px;"></i>
              <span style="color: #000000;">访客</span>
            </p>
            <p>
              统计周期内访问您店铺页面或宝贝详情页的去重人数，一个人在统计时间范围内访问多次只记为一个。所有终端访客数为PC端访客数和无线端访客数相加去重。
            </p>
        ...
        <div class="Productmarket_day">
          <van-row type="flex" justify="center">
            ...
            <van-col
              ...
              span="6"
              @click="(sel_value.Range_on.Range_set=='3')? '': Single_choice('day_7',['_this.data_screen_quota()','_this.initChart()','_this.echarts_clear=true'],true,'_this.sel_value.day_set')"
              >
              <van-button plain 
            ...
            <van-col
              ...
              span="6"
              @click="(sel_value.Range_on.Range_set=='3')? '': Single_choice('day_1',['_this.loading_data_fun()','_this.data_screen_quota()','_this.initChart()','_this.echarts_clear=true'],true,'_this.sel_value.day_set')"
              >
              <van-button plain 
      ...
      >
        ...
        <!--  -->
          ...
          >
            ...
            <span :class="sel_value.target.all_muri_value == 3? 'title_change':''" :style="sum_zhibiao.title">{{sel_value.Range_on.Range_set=='3'?  '线下':'商品访客'}}</span>
            ...
            <van-col span="8">
              ...
              <span class="target_info_text_span" >
                ...
                <i style="font-style: normal;font-size: 16px;"
                   :class="sel_value.Range_on.Range_set=='2'? Number(sel_title.quota_value.cont6.replace('%',''))>0? 'rate-up_salesR':'rate-down_salesR':''"
                >
                  {{fomatMoney(sel_title.quota_value.cont6)}}
                </i>
      ...
      this.p1.then(() => {
        ...
        try{
          ...
          if(this.sel_value.Range_on.Range_set=='1'){
            ...
            this.sel_title.chanzhi_wenzi.title_content.content3="商品访客"
            ...
            if(this.sel_value.day_set.day_1==true){
              ...
              let liuliang_data= this.data_time_select(data_timeon["FS_YY_FX-F_SALE_RATE_MONTH"],this.set_time,"push")

              liuliang_data.sort((a, b) => {
                let a_time = "";
                let b_time = "";
                a_time = a["PTD_CNT"];
                b_time = b["PTD_CNT"];
              ...
              this.sel_title.quota_value.shop_source_value=liuliang_data;
              ...
              // console.log("===============到月的！！！==================")
              // console.log(this.sel_title.quota_value.shop_source_value);
            ...
            Object.keys(user_data).forEach((key_value_chart) => {
              ...
              if(this.sel_value.day_set.month==true){
                if(user_data[key_value_chart].MDATE.substring(0,4)==this.set_time.substring(0,4)){
                  ...
                  sale_amt.push(((user_data[key_value_chart].PTD_VISITORS||0)/10000).toFixed(2));
                  ago_amt.push(((user_data[key_value_chart].PTD_DFI_PTD_VISITORS||0)/10000).toFixed(2));
                }
                ...
                sale_amt.push(((user_data[key_value_chart].PTD_VISITORS||0)/10000).toFixed(2));
                ago_amt.push(((user_data[key_value_chart].AGO_PTD_VISITORS||0)/10000).toFixed(2));
              }
            ...
            switch (this.sel_value.target.all_muri_value){
              case 1:
                Object.keys(user_data).forEach((key_value_chart) => {
                  ...
                  sale_amt.push(((user_data[key_value_chart].COMPANY_PCT||0)/10000).toFixed(2));
                  ago_amt.push(((user_data[key_value_chart].COMPANY_PCT_PREV||0)/10000).toFixed(2));
                });
              ...
              case 2:
                ...
                Object.keys(user_data).forEach((key_value_chart) => {
                  ...
                  sale_amt.push(((user_data[key_value_chart].ON_LINE_PCT||0)/10000).toFixed(2));
                  ago_amt.push(((user_data[key_value_chart].ON_LINE_PCT_PREV||0)/10000).toFixed(2));
                });
              ...
              case 3:
                Object.keys(user_data).forEach((key_value_chart) => {
                  ...
                  sale_amt.push(((user_data[key_value_chart].OFF_LINE_PCT||0)/10000).toFixed(2));
                  ago_amt.push(((user_data[key_value_chart].OFF_LINE_PCT_PREV||0)/10000).toFixed(2));
                });
                // console.log("线下");
                break;
            }
            ...
            if(this.sel_value.day_set.day_1==true||this.sel_value.day_set.day_7==true){
              ...
              Object.keys(hour_data).forEach((key_mv) => {
                ...
                if(hour_data[key_mv]==((this.set_time.substring(0,4)==_time_now.getFullYear())?((_time_now.getMonth()+1)+'月'):'12月')){
                  ...
                  this.barData.value=(sale_amt[key_mv]==undefined)?'--':sale_amt[key_mv]+'万';
                  this.barData.ago_value=(ago_amt[key_mv]==undefined)?'--':ago_amt[key_mv]+'万';
                }
              });  
          ...
          this.chartIns.setOption({
              ...
              tooltip: {
                ...
                formatter:function (params) {
                  ...
                  _this.barData.value =(params[1]==undefined||params[1].value==undefined)? '--':params[1].value+'万';
                  _this.barData.ago_value =(params[0]==undefined||params[0].value==undefined)? '--': params[0].value+'万';
