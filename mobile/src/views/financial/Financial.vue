      ...
      >
        ...
        <!-- 优化后开始 -->
          <template>
            ...
            <swiper-slide>
              <keep-alive >
                <Financialset v-if="sel_value.swiper_num==1"></Financialset> <div style="padding: 20px 0" v-else><van-skeleton title :row="10"  /></div>  
              </keep-alive>
            </swiper-slide>

            <swiper-slide>
              <keep-alive >
                <Financialset2 v-if="sel_value.swiper_num==2"></Financialset2> <div style="padding: 20px 0" v-else><van-skeleton title :row="10"  /></div>  
              </keep-alive>
            </swiper-slide>

            <swiper-slide>
              <keep-alive >
                <Financialset3 v-if="sel_value.swiper_num==3"></Financialset3> <div style="padding: 20px 0" v-else><van-skeleton title :row="10"  /></div>  
              </keep-alive>
            </swiper-slide>
...
import Financial_con from "@/views/financial/Financial_con.vue";
import Financial_con2 from "@/views/financial/Financial_con2.vue";
import Financial_con3 from "@/views/financial/Financial_con3.vue";
...
export default {
  ...
  components: {
    ...
    Financialset: Financial_con,
    Financialset2: Financial_con2,
    Financialset3: Financial_con3,
  },
  ...
  mounted(){
    ...
    this.$EventBus.$on("Salechan_choice_financial", (Range_value_set) => {
    ...
    this.$refs['mySwiper_sal'].$swiper.on("slideChangeTransitionEnd", (swiper) => {
      ...
      this.$EventBus.$emit("Salechan_choice_financial_Callback", {
    ...
    if(PermList.length==1){
      if(PermList[0]=='采购毛利'){
        this.chenSwiper(0);
      }else if(PermList[0]=='退款费用'){
        this.chenSwiper(1);
      }else if(PermList[0]=='KA达成'){
        this.chenSwiper(2);
      }
      ...
      if(PermString.indexOf("采购毛利")>=0){
        this.chenSwiper(0);
      }else if(PermString.indexOf("退款费用")>=0){
        this.chenSwiper(1);
      }
      this.JumpToPageFunCon();
    }else{
  ...
  methods: {
    JumpToPageFunCon(){
      ...
      if(this.$store.state.app.JumpToPageVal=="采购毛利"){
        this.chenSwiper(0);
      }else if(this.$store.state.app.JumpToPageVal=="退款费用"){
        this.chenSwiper(1);
      }
      // 2021-07-21增加了首页跳转
    },
    ...
    Range_Single_choice(
      ...
      if (josn_value) {
        this.$EventBus.$emit("Financial_choice_f", {
