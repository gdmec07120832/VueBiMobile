  ...
  <div class="Productmarket_all">
    ...
    <van-pull-refresh v-model="isLoading" :head-height="0" :pull-distance="0" @refresh="onRefresh">
      <div>
        ...
        >
          ...
          >
            ...
            <van-popup v-model="question_show" @close='showPopup_close' class="popup_css" get-container="body">
              ...
              <div class="ceshi1de1aaa" style="text-align: left;font-size: 14px;padding-left: 20px;
                ...
                <!-- <p> <span style="font-weight: bold;">入库体积：</span>单据类型为 SCM成品上架、定制成品上架；非定制区域   同上架单号、单据日期、物料编码、仓位  求和体积 后去重，求和总体积，
                  计数总仓位数；定制区域      同上架单号、单据日期、仓位 ，求和体积后去重，求和总体积；</p>
                <p><span style="font-weight: bold;">出库体积：</span>单据类型为：标准发货发、定制发货单、海外经销订单、天猫海外订单、异地仓调拨、异地调拨发货单等单据出库释放出的仓位数的体积汇总</p>
                <p>库存率=（可用库位数-空库位数）/可用库位数*100%</p>
                <p>当月：年化周转率(金额)=累计发出/((期初含压货+期末含压货)/2)*当年总天数/当前天数；
                  历史：年化周转率(金额)=累计发出/((期初含压货+期末含压货)/2)*12/当前月份
                  本指标只有月数据，暂无日数据</p> -->

                <van-collapse v-model="activeNames_tips" accordion>
                  <van-collapse-item title="入库体积" name="1">
                    统计时间段内入库到佛山仓库、青岛仓库的成品物料体积，含成品、定制业务数据
                  </van-collapse-item>
        ...
        <!-- 开头结束 background-color: #ffffff;class="px10 py10" display: none;-->
          ...
          <div class="Productmarket_day">
            <van-row type="flex" justify="center">
              ...
              <van-col style="margin-right: 0px;" span="6"
                ><van-button plain
                  :style="sel_value.target.all_muri_value != 4 ? {'background-color': 'rgb(255, 255, 255)'}:{'background-color': 'rgb(247, 247, 247)'}"
                  @click="sel_value.target.all_muri_value != 4 ?
                    Single_choice('day_7',['_this.data_Logistics()','_this.initChart()','_this.initChart2()','_this.echarts_clear=true'],true,'_this.sel_value.day_set'):''"
                  :type="
                ...
                ></van-col
              >
              <van-col style="margin-right: 10px;margin-left: 10px;" 
                span="6"
                ><van-button plain
                  :style="sel_value.target.all_muri_value != 4 ? {'background-color': 'rgb(255, 255, 255)'}:{'background-color': 'rgb(247, 247, 247)'}"
                  @click="sel_value.target.all_muri_value != 4 ?
                    Single_choice('day_30',['_this.data_Logistics()','_this.initChart()','_this.initChart2()','_this.echarts_clear=true'],true,'_this.sel_value.day_set'):''"
                  :type="
                    sel_value.target.all_muri_value != 4 ?
        ...
        <section
          style="
            ...
            margin-right: 10px;
          "
          :style="(sel_value.Range_on.Range_set=='11')?{'display':'block'}:{'display':'none'}"
        >
              ...
              <swiper  ref="mySwiper_entry" :options="swiperOptions">
                <template>
                  <swiper-slide >
                    <div
                      @click="
                        Single_choice('all_muri_value',['_this.loading_data_fun()','_this.data_Logistics()','_this.initChart()','_this.initChart2()','_this.echarts_clear=true'], 1, '_this.sel_value.target')
                      "
                    ...
                    >
                      ...
                      <span :style="sum_zhibiao.zhognxin">
                        {{nll_undefined(fomatMoney((sel_title.quota_value.cont7).toFixed(1)))+'m³'}}
                      </span>

                      <span class="yoy_tb" :style="sum_zhibiao.body">
                        <i style="width: 31%; display: inline-block; font-style: normal"
                          >同比</i
                        >
                        <i style="width: 69%; font-style: normal;" ref="fhyoy_h"
                        :class="Number(sel_title.quota_value.cont8) > 0? 'rate-up': 'rate-down'"
                        
                        > 
                        
                          {{nll_undefined(((sel_title.quota_value.cont8||0)*100).toFixed(1))+'%'}}
                        </i>
                      </span>
                      ...
                      <span class="yoy_tb" :style="sum_zhibiao.body">
                        <i style="width: 31%; display: inline-block; font-style: normal"
                          >环比</i
                        >
                        <i style="width: 69%; font-style: normal;" ref="fhhb_h"
                        :class="Number(sel_title.quota_value.cont9) > 0? 'rate-up': 'rate-down'"
                        > 
                          {{nll_undefined(((sel_title.quota_value.cont9||0)*100).toFixed(1))+'%'}}
                        </i>
                      </span>
                    ...
                    <div
                      @click="
                        Single_choice('all_muri_value',['_this.loading_data_fun()','_this.data_Logistics()','_this.initChart()','_this.initChart2()','_this.echarts_clear=true'], 2, '_this.sel_value.target')
                      "
                    ...
                    >
                      ...
                      >
                      <span :style="sum_zhibiao.zhognxin">{{
                          nll_undefined(fomatMoney((sel_title.quota_value.cont10).toFixed(1)))+'m³'
                        }}</span>
                      ...
                      <span class="yoy_tb" :style="sum_zhibiao.body">
                        <i style="width: 31%; display: inline-block; font-style: normal"
                          >同比</i
                        >
                        <i style="width: 69%; font-style: normal;" ref="fhyoy_h2"
                        :class="Number(sel_title.quota_value.cont11) > 0? 'rate-up': 'rate-down'"
                        >
                          {{nll_undefined(((sel_title.quota_value.cont11||0)*100).toFixed(1))+'%'}}
                        </i>
                      </span>
                      ...
                      <span class="yoy_tb" :style="sum_zhibiao.body">
                        <i style="width: 31%; display: inline-block; font-style: normal"
                          >环比</i
                        >
                        <i style="width: 69%; font-style: normal;" ref="fhhb_h2"
                        :class="Number(sel_title.quota_value.cont12) > 0? 'rate-up': 'rate-down'"
                        > 
                          {{nll_undefined(((sel_title.quota_value.cont12||0)*100).toFixed(1))+'%'}}
                        </i>
                      </span>
                    ...
                    <div
                      @click="
                        Single_choice('all_muri_value',['_this.loading_data_fun()','_this.data_Logistics()','_this.initChart()','_this.initChart2()','_this.echarts_clear=true'], 3, '_this.sel_value.target')
                      "
                    ...
                    >
                      ...
                      >
                      <span :style="sum_zhibiao.zhognxin">{{
                          nll_undefined(fomatMoney((sel_title.quota_value.cont1*100).toFixed(1))) + '%'
                        }}</span>
                      ...
                      <span class="yoy_tb" :style="sum_zhibiao.body">
                        ...
                        <i style="width: 69%; font-style: normal;padding-left: 3px;" ref="fhyoy_h3"
                      ...
                      <span class="yoy_tb" :style="sum_zhibiao.body">
                        <i style="width: 31%; display: inline-block; font-style: normal"
                          >环比</i
                        >
                        <i style="width: 69%; font-style: normal;padding-left: 3px;" ref="fhhb_h3"
                        :class="Number(sel_title.quota_value.cont3) > 0? 'rate-up': 'rate-down'"
                        > 
                          {{nll_undefined(((sel_title.quota_value.cont3||0)*100).toFixed(1))+'%'}}
                        </i>
                      </span>
                  ...
                  <swiper-slide >
                    ...
                    <div
                      @click="
                        Single_choice('all_muri_value',['_this.loading_data_fun()','_this.data_Logistics()','_this.initChart()','_this.initChart2()','_this.echarts_clear=true'], 4, '_this.sel_value.target')
                      "
                      class="Productmarket_labqie_con3"
                      :style="
                        sel_value.target.all_muri_value == 4
                    ...
                    >
                      ...
                      >
                      <span :style="sum_zhibiao.zhognxin">{{
                          nll_undefined(fomatMoney(sel_title.quota_value.cont4.toFixed(1)))
                        }}</span>
                      ...
                      <span class="yoy_tb" :style="sum_zhibiao.body">
                        <i style="width: 31%; display: inline-block; font-style: normal"
                          >同比</i
                        >
                        <i style="width: 69%; font-style: normal;padding-left: 3px;" ref="fhyoy_h4"
                        :class="Number(sel_title.quota_value.cont5) > 0? 'rate-up': 'rate-down'"
                        >
                          {{nll_undefined(((sel_title.quota_value.cont5||0)*100).toFixed(1))+'%'}}
                        </i>
                      </span>
                      ...
                      <span class="yoy_tb" :style="sum_zhibiao.body">
                        <i style="width: 31%; display: inline-block; font-style: normal"
                          >环比</i
                        >
                        <i style="width: 69%; font-style: normal;padding-left: 3px;" ref="fhhb_h4"
                ...
                <van-col span="8">
                  <div>
                    <div style="margin: 0 auto; width: 50px; height: 17px;">
                      ...
                      <div style="font-size: 12px;"><span style="line-height: 18px;">{{sel_value.target.all_muri_value==1?'当期'
                        :sel_value.target.all_muri_value==2?'当期'
                        :sel_value.target.all_muri_value==3?'当期'
                        :sel_value.target.all_muri_value==4?'当期':''}}</span></div>
                    </div>
                    <div style="text-align: center;">
        ...
        <section
          style="
            ...
            padding-right: 10px;
          "
          :style="(sel_value.Range_on.Range_set=='111')?{'display':'block'}:{'display':'none'}"
        >
          <div
          ...
          >
            ...
            <section class="px10 py10" :style="(sel_value.Range_on.Range_set=='2'||sel_value.Range_on.Range_set=='5')? echarts2_css_n : echarts2_css_y">
              <ul class="table_all">
                ...
                <li
                  class="table_content"
                  v-for="(val, key) in sel_title_setvalue.set_value_stock"
                  :key="key"
                >
                  <ul>
                    <li>{{val.name}}</li>
                    <li>{{
                      sel_value.target.all_muri_value==1||sel_value.target.all_muri_value==2?
                      fomatMoney(((val.fs_lov||0)).toFixed(1))+'m³'
                      :sel_value.target.all_muri_value==3?
                      fomatMoney(((val.fs_lov||0)*100).toFixed(1))+'%'
                      :fomatMoney(((val.fs_lov||0)).toFixed(1))+''
                      }}</li>

                    <li>{{
                      sel_value.target.all_muri_value==1||sel_value.target.all_muri_value==2?
                      fomatMoney(((val.fs_zb||0)*100).toFixed(1))+'%'
                      :'--'
                      }}</li>
                    ...
                    <li>{{
                      sel_value.target.all_muri_value==1||sel_value.target.all_muri_value==2?
                      fomatMoney(((val.fs_ly||0)).toFixed(1))+'m³'
                      :sel_value.target.all_muri_value==3?
                      fomatMoney(((val.fs_ly||0)*100).toFixed(1))+'%'
                      :fomatMoney(((val.fs_ly||0)).toFixed(1))+''

                      }}</li>


                    <li
                    :class="(Number(val.fs_yoy||0)>=0?'rate-up_sales':'rate-down_sales')"
                    >
                      {{((val.fs_yoy||0)*100).toFixed(1)+'%'}}
                    </li>
...
export default {
  ...
  created() {
    ...
    const apiMap2 = {
      ...
      // "SQL-LC_INV-OUTSTOCK_AMT":{resultProp:"OUTSTOCK_AMT"},  //已经没有的了
      "SQL-LC_INV-STOCK_AMT":{resultProp:"STOCK_AMT"},
      // "SQL-LC_INV-STOCK_NAME_AMT":{resultProp:"STOCK_NAME_AMT"},  //已经没有用的了
      // "SQL-LC_PL-DLVR_COST":{resultProp:"DLVR_COST"},
      // "SQL-LC_PL-DLVR_COST_CHNL":{resultProp:"DLVR_COST_CHNL"},

      "SQL-LC_INV-IN_OUT_INV_VOL_D":{resultProp:"IN_OUT_INV_VOL_D"},
      "SQL-LC_INV-STOCKLOC_RATIO":{resultProp:"STOCKLOC_RATIO"},
      "SQL-LC_PL-TURNOVER_RATE":{resultProp:"TURNOVER_RATE"},


      // "SQL-LC_INV-IN_OUT_INV_VOL_M":{resultProp:"IN_OUT_INV_VOL_M"},   //月      
      // "SQL-LC_PL-LC_PERIOD_STOCK":{resultProp:"LC_PERIOD_STOCK"},   //月      
      // "SQL-LC_INV-STOCKLOC_RATIO_M":{resultProp:"STOCKLOC_RATIO_M"},   //月
  ...
  mounted() {
    ...
    this.Single_choice('all_muri_value',['_this.initChart()'], 1, '_this.sel_value.target')
    this.initChart2();
  ...
  methods: {
    ...
    onRefresh() {
      ...
      let apiMap3 = {
        ...
        // "SQL-LC_INV-OUTSTOCK_AMT":{resultProp:"OUTSTOCK_AMT"},  //已经没有的了
        "SQL-LC_INV-STOCK_AMT":{resultProp:"STOCK_AMT"},
        // "SQL-LC_INV-STOCK_NAME_AMT":{resultProp:"STOCK_NAME_AMT"},  //已经没用的了
        "SQL-LC_INV-IN_OUT_INV_VOL_D":{resultProp:"IN_OUT_INV_VOL_D"},
        "SQL-LC_INV-STOCKLOC_RATIO":{resultProp:"STOCKLOC_RATIO"},
        "SQL-LC_PL-TURNOVER_RATE":{resultProp:"TURNOVER_RATE"},
      };
    ...
    onConfirm(value, index) {
      ...
      this.Single_choice('month',['_this.loading_data_fun()','_this.data_Logistics()','_this.initChart()','_this.initChart2()','_this.echarts_clear=true'],true,'_this.sel_value.day_set')
    ...
    data_Logistics() {
        ...
        let set_time_month=this.set_time;
      ...
      this.p1.then(() => {
        ...
        if(this.sel_value.Range_on.Range_set=='1'){
          ...
          if(this.sel_value.day_set.day_7==true||this.sel_value.day_set.day_30==true){
            ...
            let kcje_val=this.data_time_select(this.data["LC_INV-STOCKLOC_RATIO"],time_t1_on,'','TDATE_WID',8);
            this.sel_title.quota_value.cont1 = kcje_val.STOCKLOC_RATIO||0;
            this.sel_title.quota_value.cont2 = kcje_val.YOY_STOCKLOC_RATIO||0;
            this.sel_title.quota_value.cont3 = kcje_val.MOM_STOCKLOC_RATIO||0; 

            // console.log("==========库存吞吐量！！！！！===========");
            // console.log(this.data["LC_PL-TURNOVER_RATE"]);  
            // console.log(kcje_val);
            // console.log(this.set_time.substring(0,6));  

            // "！！周转率"  
            let zzl_val=this.data_time_select(this.data["LC_PL-TURNOVER_RATE"],time_t1_on.substring(0,6),'','TMONTH_WID',6);
            ...
            // console.log("==asd==");
            // console.log(this.data["LC_INV-STOCKLOC_RATIO_M"]);
            // console.log(this.data);
            // console.log(this.data["LC_INV-STOCKLOC_RATIO_M"][0].TMONTH.substring(0,7));

            let kcje_val=this.data_time_select(this.data["LC_INV-STOCKLOC_RATIO_M"],(set_time_month.substring(0,4)+'-'+set_time_month.substring(5,7)),'','TMONTH',7);
            this.sel_title.quota_value.cont1 = kcje_val.STOCKLOC_RATIO||0;
            this.sel_title.quota_value.cont2 = kcje_val.YOY_STOCKLOC_RATIO||0;
            this.sel_title.quota_value.cont3 = kcje_val.MOM_STOCKLOC_RATIO||0; 

            // "！！周转率"  
            let zzl_val=this.data_time_select(this.data["LC_PL-TURNOVER_RATE"],(set_time_month.substring(0,4)+''+set_time_month.substring(5,7)),'','TMONTH_WID',6);
            // this.data["LC_PL-TURNOVER_RATE"][0].STOCK_AMT || 0;
            this.sel_title.quota_value.cont4 =zzl_val.TM_TURNOVER_RATE||0;
            this.sel_title.quota_value.cont5 = zzl_val.YOY_TURNOVER_RATE||0;
            this.sel_title.quota_value.cont6 = zzl_val.MOM_TURNOVER_RATE||0; 
          ...
          if(this.sel_value.day_set.day_7==true){
            let crk_tz=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_D"],time_t1_on,'','TDATE_WID',8);
            this.sel_title.quota_value.cont7 = crk_tz.TP_INSTOCK_VOL_7D || 0;
            this.sel_title.quota_value.cont8 = crk_tz.YOY_INSTOCK_VOL_7D || 0;
            this.sel_title.quota_value.cont9 = crk_tz.POP_INSTOCK_VOL_7D ||0;
            this.sel_title.quota_value.cont10 = crk_tz.TP_OUTSTOCK_VOL_7D || 0;
            this.sel_title.quota_value.cont11 = crk_tz.YOY_OUTSTOCK_VOL_7D || 0;
            this.sel_title.quota_value.cont12 = crk_tz.POP_OUTSTOCK_VOL_7D || 0;



            // this.sel_title_setvalue.set_value_stock=
            //   [{name:'佛山仓',fs_lov:crk_tz.TP_INSTOCK_VOL_FS_7D,fs_zb:crk_tz.TP_INSTOCK_RATE_FS_7D,fs_ly:crk_tz.LY_INSTOCK_VOL_FS_7D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_FS_7D},
            //    {name:'青岛仓',fs_lov:crk_tz.TP_INSTOCK_VOL_QD_7D,fs_zb:crk_tz.TP_INSTOCK_RATE_QD_7D,fs_ly:crk_tz.LY_INSTOCK_VOL_QD_7D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_QD_7D},
            //    {name:'总仓',fs_lov:crk_tz.TP_INSTOCK_VOL_7D,fs_zb:0,fs_ly:crk_tz.LY_INSTOCK_VOL_7D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_7D}];


          }else if(this.sel_value.day_set.day_30==true){
            let crk_tz=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_D"],time_t1_on,'','TDATE_WID',8);
            this.sel_title.quota_value.cont7 = crk_tz.TP_INSTOCK_VOL_30D || 0;
            this.sel_title.quota_value.cont8 = crk_tz.YOY_INSTOCK_VOL_30D || 0;
            this.sel_title.quota_value.cont9 = crk_tz.POP_INSTOCK_VOL_30D ||0;
            this.sel_title.quota_value.cont10 = crk_tz.TP_OUTSTOCK_VOL_30D || 0;
            this.sel_title.quota_value.cont11 = crk_tz.YOY_OUTSTOCK_VOL_30D || 0;
            this.sel_title.quota_value.cont12 = crk_tz.POP_OUTSTOCK_VOL_30D || 0;
          }else if(this.sel_value.day_set.month==true){

            let crk_tz=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_M"],(set_time_month.substring(0,4)+''+set_time_month.substring(5,7)),'','TMONTH_WID',6);
            this.sel_title.quota_value.cont7 = crk_tz.INSTOCK_VOL || 0;
            this.sel_title.quota_value.cont8 = crk_tz.YOY_INSTOCK_VOL || 0;
            this.sel_title.quota_value.cont9 = crk_tz.MOM_INSTOCK_VOL ||0;
            this.sel_title.quota_value.cont10 = crk_tz.OUTSTOCK_VOL || 0;
            this.sel_title.quota_value.cont11 = crk_tz.YOY_OUTSTOCK_VOL || 0;
            this.sel_title.quota_value.cont12 = crk_tz.MOM_OUTSTOCK_VOL || 0;
            
          }


          if(this.sel_value.target.all_muri_value==4){
            this.Single_choice('month','',true,'_this.sel_value.day_set')
          }
          ...
          if(this.sel_value.target.all_muri_value==1){
            ...
            if(this.sel_value.day_set.day_7==true){
              ...
              [{name:'佛山仓',fs_lov:crk_tz.TP_INSTOCK_VOL_FS_7D,fs_zb:crk_tz.TP_INSTOCK_RATE_FS_7D,fs_ly:crk_tz.LY_INSTOCK_VOL_FS_7D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_FS_7D},
               {name:'青岛仓',fs_lov:crk_tz.TP_INSTOCK_VOL_QD_7D,fs_zb:crk_tz.TP_INSTOCK_RATE_QD_7D,fs_ly:crk_tz.LY_INSTOCK_VOL_QD_7D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_QD_7D},
               {name:'总仓',fs_lov:crk_tz.TP_INSTOCK_VOL_7D,fs_zb:1,fs_ly:crk_tz.LY_INSTOCK_VOL_7D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_7D}];

            }else if(this.sel_value.day_set.day_30==true){
              let crk_tz=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_D"],time_t1_on,'','TDATE_WID',8);
              this.sel_title_setvalue.set_value_stock=
              [{name:'佛山仓',fs_lov:crk_tz.TP_INSTOCK_VOL_FS_30D,fs_zb:crk_tz.TP_INSTOCK_RATE_FS_30D,fs_ly:crk_tz.LY_INSTOCK_VOL_FS_30D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_FS_30D},
               {name:'青岛仓',fs_lov:crk_tz.TP_INSTOCK_VOL_QD_30D,fs_zb:crk_tz.TP_INSTOCK_RATE_QD_30D,fs_ly:crk_tz.LY_INSTOCK_VOL_QD_30D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_QD_30D},
               {name:'总仓',fs_lov:crk_tz.TP_INSTOCK_VOL_30D,fs_zb:1,fs_ly:crk_tz.LY_INSTOCK_VOL_30D,fs_yoy:crk_tz.YOY_INSTOCK_VOL_30D}];

            }else if(this.sel_value.day_set.month==true){
              let crk_tz=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_M"],(set_time_month.substring(0,4)+''+set_time_month.substring(5,7)),'','TMONTH_WID',6);
              this.sel_title_setvalue.set_value_stock=
              [{name:'佛山仓',fs_lov:crk_tz.INSTOCK_VOL_FS,fs_zb:crk_tz.INSTOCK_RATE_FS,fs_ly:crk_tz.LY_INSTOCK_VOL_FS,fs_yoy:crk_tz.YOY_INSTOCK_VOL_FS},
               {name:'青岛仓',fs_lov:crk_tz.INSTOCK_VOL_QD,fs_zb:crk_tz.INSTOCK_RATE_QD,fs_ly:crk_tz.LY_INSTOCK_VOL_QD,fs_yoy:crk_tz.YOY_INSTOCK_VOL_QD},
               {name:'总仓',fs_lov:crk_tz.INSTOCK_VOL,fs_zb:1,fs_ly:crk_tz.LY_INSTOCK_VOL,fs_yoy:crk_tz.YOY_INSTOCK_VOL}];
            }
            ...
            if(this.sel_value.day_set.day_7==true){
              let crk_tz=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_D"],time_t1_on,'','TDATE_WID',8);
              this.sel_title_setvalue.set_value_stock=
              [{name:'佛山仓',fs_lov:crk_tz.TP_OUTSTOCK_VOL_FS_7D,fs_zb:crk_tz.TP_OUTSTOCK_RATE_FS_7D,fs_ly:crk_tz.LY_OUTSTOCK_VOL_FS_7D,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL_FS_7D},
               {name:'青岛仓',fs_lov:crk_tz.TP_OUTSTOCK_VOL_QD_7D,fs_zb:crk_tz.TP_OUTSTOCK_RATE_QD_7D,fs_ly:crk_tz.LY_OUTSTOCK_VOL_QD_7D,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL_QD_7D},
               {name:'总仓',fs_lov:crk_tz.TP_OUTSTOCK_VOL_7D,fs_zb:1,fs_ly:crk_tz.LY_OUTSTOCK_VOL_7D,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL_7D}];

            }else if(this.sel_value.day_set.day_30==true){
              let crk_tz=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_D"],time_t1_on,'','TDATE_WID',8);
              this.sel_title_setvalue.set_value_stock=
              [{name:'佛山仓',fs_lov:crk_tz.TP_OUTSTOCK_VOL_FS_30D,fs_zb:crk_tz.TP_OUTSTOCK_RATE_FS_30D,fs_ly:crk_tz.LY_OUTSTOCK_VOL_FS_30D,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL_FS_30D},
               {name:'青岛仓',fs_lov:crk_tz.TP_OUTSTOCK_VOL_QD_30D,fs_zb:crk_tz.TP_OUTSTOCK_RATE_QD_30D,fs_ly:crk_tz.LY_OUTSTOCK_VOL_QD_30D,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL_QD_30D},
               {name:'总仓',fs_lov:crk_tz.TP_OUTSTOCK_VOL_30D,fs_zb:1,fs_ly:crk_tz.LY_OUTSTOCK_VOL_30D,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL_30D}];


            }else if(this.sel_value.day_set.month==true){
              let crk_tz=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_M"],(set_time_month.substring(0,4)+''+set_time_month.substring(5,7)),'','TMONTH_WID',6);
              this.sel_title_setvalue.set_value_stock=
              [{name:'佛山仓',fs_lov:crk_tz.OUTSTOCK_VOL_FS,fs_zb:crk_tz.OUTSTOCK_RATE_FS,fs_ly:crk_tz.LY_OUTSTOCK_VOL_FS,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL_FS},
               {name:'青岛仓',fs_lov:crk_tz.OUTSTOCK_VOL_QD,fs_zb:crk_tz.OUTSTOCK_RATE_QD,fs_ly:crk_tz.LY_OUTSTOCK_VOL_QD,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL_QD},
               {name:'总仓',fs_lov:crk_tz.OUTSTOCK_VOL,fs_zb:1,fs_ly:crk_tz.LY_OUTSTOCK_VOL,fs_yoy:crk_tz.YOY_OUTSTOCK_VOL}];
            }
            ...
            if(this.sel_value.day_set.day_7==true||this.sel_value.day_set.day_30==true){
              let kcje_val=this.data_time_select(this.data["LC_INV-STOCKLOC_RATIO"],time_t1_on,'','TDATE_WID',8);
              this.sel_title_setvalue.set_value_stock=
              [{name:'佛山仓',fs_lov:kcje_val.STOCKLOC_RATIO_FS,fs_zb:0,fs_ly:kcje_val.LY_STOCKLOC_RATIO_FS,fs_yoy:kcje_val.YOY_STOCKLOC_RATIO_FS},
               {name:'青岛仓',fs_lov:kcje_val.STOCKLOC_RATIO_QD,fs_zb:0,fs_ly:kcje_val.LY_STOCKLOC_RATIO_QD,fs_yoy:kcje_val.YOY_STOCKLOC_RATIO_QD},
               {name:'总仓',fs_lov:kcje_val.STOCKLOC_RATIO,fs_zb:0,fs_ly:kcje_val.LY_STOCKLOC_RATIO,fs_yoy:kcje_val.YOY_STOCKLOC_RATIO}];
            }else if(this.sel_value.day_set.month==true){
              let kcje_val=this.data_time_select(this.data["LC_INV-STOCKLOC_RATIO_M"],(set_time_month.substring(0,4)+'-'+set_time_month.substring(5,7)),'','TMONTH',7);
              this.sel_title_setvalue.set_value_stock=
              [{name:'佛山仓',fs_lov:kcje_val.STOCKLOC_RATIO_FS,fs_zb:0,fs_ly:kcje_val.LY_STOCKLOC_RATIO_FS,fs_yoy:kcje_val.YOY_STOCKLOC_RATIO_FS},
            ...
            if(this.sel_value.day_set.month==true){
              let zzl_val=this.data_time_select(this.data["LC_PL-TURNOVER_RATE"],(set_time_month.substring(0,4)+''+set_time_month.substring(5,7)),'','TMONTH_WID',6);
              this.sel_title_setvalue.set_value_stock=
              [{name:'佛山仓',fs_lov:zzl_val.TM_TURNOVER_RATE_FS,fs_zb:0,fs_ly:zzl_val.LYTM_TURNOVER_RATE_FS,fs_yoy:zzl_val.YOY_TURNOVER_RATE_FS},
              {name:'青岛仓',fs_lov:zzl_val.TM_TURNOVER_RATE_QD,fs_zb:0,fs_ly:zzl_val.LYTM_TURNOVER_RATE_QD,fs_yoy:zzl_val.YOY_TURNOVER_RATE_QD},
              {name:'总仓',fs_lov:zzl_val.TM_TURNOVER_RATE,fs_zb:0,fs_ly:zzl_val.LYTM_TURNOVER_RATE,fs_yoy:zzl_val.YOY_TURNOVER_RATE}]; 
            }
          ...
          // console.log("运行到数据填充函数！=======  adadasdasdad"); up_down_width
          // console.log(this.$refs["fhyoy_h"].innerText)
          // this.$nextTick(()=>{
          //  this.up_down_width.set_on_width=(this.$refs["fhyoy_h"].offsetWidth>this.$refs["fhhb_h"].offsetWidth)?this.$refs["fhyoy_h"].offsetWidth:this.$refs["fhhb_h"].offsetWidth;
          //  this.up_down_width.set_on_width2=(this.$refs["fhyoy_h2"].offsetWidth>this.$refs["fhhb_h2"].offsetWidth)?this.$refs["fhyoy_h2"].offsetWidth:this.$refs["fhhb_h2"].offsetWidth;
          //  this.up_down_width.set_on_width3=(this.$refs["fhyoy_h3"].offsetWidth>this.$refs["fhhb_h3"].offsetWidth)?this.$refs["fhyoy_h3"].offsetWidth:this.$refs["fhhb_h3"].offsetWidth;
          //  this.up_down_width.set_on_width4=(this.$refs["fhyoy_h4"].offsetWidth>this.$refs["fhhb_h4"].offsetWidth)?this.$refs["fhyoy_h4"].offsetWidth:this.$refs["fhhb_h4"].offsetWidth;
          //   console.log(this.up_down_width.set_on_width);
          //   console.log(this.up_down_width.set_on_width2);
          //   console.log(this.up_down_width.set_on_width3);
          //   console.log(this.up_down_width.set_on_width4);
          // })
    ...
    initChart() {
      ...
      this.p1.then(() => {
        ...
        // console.log(this.data["LC_INV-STOCKLOC_RATIO"]);
        ...
        if(this.sel_value.Range_on.Range_set=='1'){
          ...
          if(this.sel_value.day_set.day_7==true){
            if(this.sel_value.target.all_muri_value==1){ 
              ...
              user_data=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_D"],time_set_on,'push','TDATE_WID',8,time_t1_on_userday);   
              user_data.sort((a, b) => {
                  let a_time = "";
                  let b_time = "";
              ...
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TDATE_WID.substring(4,6)+'.'+user_data[key_value].TDATE_WID.substring(6,8));
                // sale_amt.push((user_data[key_value].TP_INSTOCK_VOL).toFixed(1));
                sale_amt_fs.push((user_data[key_value].TP_INSTOCK_VOL_FS).toFixed(1));
                sale_amt_qd.push((user_data[key_value].TP_INSTOCK_VOL_QD).toFixed(1));
              ...
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TDATE_WID.substring(4,6)+'.'+user_data[key_value].TDATE_WID.substring(6,8));
                // sale_amt.push((user_data[key_value].TP_OUTSTOCK_VOL).toFixed(1));
                sale_amt_fs.push((user_data[key_value].TP_OUTSTOCK_VOL_FS).toFixed(1));
                sale_amt_qd.push((user_data[key_value].TP_OUTSTOCK_VOL_QD).toFixed(1));
              ...
              user_data=this.data_time_select(this.data["LC_INV-STOCKLOC_RATIO"],time_set_on,'push','TDATE_WID',8,time_t1_on_userday);
              ...
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TDATE_WID.substring(4,6)+'.'+user_data[key_value].TDATE_WID.substring(6,8));
                // sale_amt.push((user_data[key_value].STOCKLOC_RATIO).toFixed(1));
                sale_amt_fs.push((user_data[key_value].STOCKLOC_RATIO_FS*100).toFixed(1));
                sale_amt_qd.push((user_data[key_value].STOCKLOC_RATIO_QD*100).toFixed(1));

              });

            }else if(this.sel_value.target.all_muri_value==4){

              // user_data=this.data_time_select(this.data["LC_PL-TURNOVER_RATE"],this.set_time.substring(0,6),'','TMONTH_WID',6);
              // hour_data=["总仓","佛山仓","青岛仓"];
              // sale_amt=[user_data.TM_TURNOVER_RATE,user_data.TM_TURNOVER_RATE_FS,user_data.TM_TURNOVER_RATE_QD]; 

              user_data=this.data_time_select(this.data["LC_PL-TURNOVER_RATE"],time_set_on.substring(0,4),'push','TMONTH_WID',4);
              user_data.sort((a, b) => {
                  let a_time = "";
                  let b_time = "";
                  a_time = new Date(a["TMONTH_WID"].substring(0,10)).getTime(new Date(a["TMONTH_WID"].substring(0,10)));
                  b_time = new Date(b["TMONTH_WID"].substring(0,10)).getTime(new Date(b["TMONTH_WID"].substring(0,10)));
                  return a_time - b_time;
              });
            ...
            if(this.sel_value.target.all_muri_value==1){    
              ...
              user_data=this.data_time_select(this.data["LC_INV-IN_OUT_INV_VOL_M"],this.set_time.toString(0,4),'push','TMONTH_WID',4);   
              user_data.sort((a, b) => {
                  let a_time = "";
                  let b_time = "";
              ...
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TMONTH_WID.substring(4,6)*1+'月');
                // sale_amt.push((user_data[key_value].INSTOCK_VOL).toFixed(1));
                sale_amt_fs.push((user_data[key_value].INSTOCK_VOL_FS).toFixed(1));
                sale_amt_qd.push((user_data[key_value].INSTOCK_VOL_QD).toFixed(1));
              ...
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TMONTH_WID.substring(4,6)*1+'月');
                // sale_amt.push((user_data[key_value].OUTSTOCK_VOL).toFixed(1));
                sale_amt_fs.push((user_data[key_value].OUTSTOCK_VOL_FS).toFixed(1));
                sale_amt_qd.push((user_data[key_value].OUTSTOCK_VOL_QD).toFixed(1));
              ...
              user_data=this.data_time_select(this.data["LC_INV-STOCKLOC_RATIO_M"],this.set_time.toString(0,4),'push','TMONTH',4);
              user_data.sort((a, b) => {
                  let a_time = "";
                  let b_time = "";
                  a_time = new Date(a["TMONTH"].substring(0,10)).getTime(new Date(a["TMONTH"].substring(0,10)));
              ...
              console.log(user_data);
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TDATE_WID.substring(4,6)*1+'月');
                // sale_amt.push((user_data[key_value].STOCKLOC_RATIO).toFixed(1));
                sale_amt_fs.push((user_data[key_value].STOCKLOC_RATIO_FS*100).toFixed(1));
                sale_amt_qd.push((user_data[key_value].STOCKLOC_RATIO_QD*100).toFixed(1));
              ...
              user_data=this.data_time_select(this.data["LC_PL-TURNOVER_RATE"],this.set_time.substring(0,4),'push','TMONTH_WID',4);
              ...
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TMONTH_WID.substring(4,6)*1+'月');
                // sale_amt.push((user_data[key_value].TM_TURNOVER_RATE).toFixed(1));
                sale_amt_fs.push((user_data[key_value].TM_TURNOVER_RATE_FS).toFixed(1));
                sale_amt_qd.push((user_data[key_value].TM_TURNOVER_RATE_QD).toFixed(1));
        ...
        this.chartIns.setOption({
          ...
          tooltip: {
            ...
            formatter:function (params) {
              let time_month_tip=params[0].axisValue;
              let fs_info=params[0].marker+params[0].seriesName+'：'+params[0].value+
                (_this.sel_value.target.all_muri_value==1||_this.sel_value.target.all_muri_value==2?'m³'
                :_this.sel_value.target.all_muri_value==3?'%':'');
              let qd_info=params[1].marker+params[1].seriesName+'：'+params[1].value+
                (_this.sel_value.target.all_muri_value==1||_this.sel_value.target.all_muri_value==2?'m³'
                :_this.sel_value.target.all_muri_value==3?'%':'');
              return time_month_tip+'<br/>'+fs_info+'<br/>'+qd_info;
            }
          ...
          series: [
            {
              ...
              barWidth:sale_amt.length>4? '90%':'60%',
              data: sale_amt,
              type: "bar",
              areaStyle: {},
...

            {
              ...
              barWidth:sale_amt_fs.length>4? '90%':'60%',
              data: sale_amt_fs,
              type: "bar",
              areaStyle: {},
...

            {
              ...
              barWidth:sale_amt_qd.length>4? '90%':'60%',
              data: sale_amt_qd,
              type: "bar",
              areaStyle: {},
    ...
    initChart2() {
      ...
      this.p1.then(() => {
          ...
          if(this.sel_value.day_set.day_7==true){
            ...
            if(this.sel_value.target.all_muri_value==1){
              ...
              Object.keys(user_data).forEach((key_value2) => {
                hour_data.push(user_data[key_value2].TDATE_WID.substring(4,6)+'.'+user_data[key_value2].TDATE_WID.substring(6,8));
                year_add_time.push(user_data[key_value2].TDATE_WID.substring(0,4));  
                sale_amt.push((user_data[key_value2].TP_INSTOCK_VOL).toFixed(1)); 
                ago_amt.push((user_data[key_value2].LY_INSTOCK_VOL).toFixed(1));
              ...
              Object.keys(user_data).forEach((key_value2) => {
                hour_data.push(user_data[key_value2].TDATE_WID.substring(4,6)+'.'+user_data[key_value2].TDATE_WID.substring(6,8));
                year_add_time.push(user_data[key_value2].TDATE_WID.substring(0,4));  
                sale_amt.push((user_data[key_value2].TP_OUTSTOCK_VOL).toFixed(1));
                ago_amt.push((user_data[key_value2].LY_OUTSTOCK_VOL).toFixed(1));
              });
              ...
              user_data=this.data_time_select(this.data["LC_INV-STOCKLOC_RATIO"],time_set_on,'push','TDATE_WID',8,time_t1_on_userday);
              // hour_data=["总仓","佛山仓","青岛仓"];
              // sale_amt=[user_data.STOCKLOC_RATIO,user_data.STOCKLOC_RATIO_FS,user_data.STOCKLOC_RATIO_QD]; 
              user_data.sort((a, b) => {
                  let a_time = "";
                  let b_time = "";
                  a_time = new Date(a["TDATE"].substring(0,10)).getTime(new Date(a["TDATE"].substring(0,10)));
              ...
              Object.keys(user_data).forEach((key_value2) => {
                hour_data.push(user_data[key_value2].TDATE_WID.substring(4,6)+'.'+user_data[key_value2].TDATE_WID.substring(6,8));
                year_add_time.push(user_data[key_value2].TDATE_WID.substring(0,4));  
                sale_amt.push((user_data[key_value2].STOCKLOC_RATIO*100).toFixed(1));
                ago_amt.push((user_data[key_value2].LY_STOCKLOC_RATIO*100).toFixed(1));
              });
            ...
            if(this.sel_value.target.all_muri_value==1){    
              ...
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TMONTH_WID.substring(4,6)*1+'月');
                year_add_time.push(user_data[key_value].TMONTH_WID.substring(0,4));  
                sale_amt.push((user_data[key_value].INSTOCK_VOL).toFixed(1));
                ago_amt.push((user_data[key_value].LY_INSTOCK_VOL).toFixed(1));
              ...
              Object.keys(user_data).forEach((key_value) => {
                ...
                year_add_time.push(user_data[key_value].TMONTH_WID.substring(0,4));  
                sale_amt.push((user_data[key_value].OUTSTOCK_VOL).toFixed(1));
                ago_amt.push((user_data[key_value].LY_OUTSTOCK_VOL).toFixed(1));
              ...
              Object.keys(user_data).forEach((key_value) => {
                hour_data.push(user_data[key_value].TDATE_WID.substring(4,6)*1+'月');
                year_add_time.push(user_data[key_value].TDATE_WID.substring(0,4));  
                sale_amt.push((user_data[key_value].STOCKLOC_RATIO*100).toFixed(1));
                ago_amt.push((user_data[key_value].LY_STOCKLOC_RATIO*100).toFixed(1));
              ...
              Object.keys(user_data).forEach((key_value) => {
                ...
                year_add_time.push(user_data[key_value].TMONTH_WID.substring(0,4));  
                sale_amt.push((user_data[key_value].TM_TURNOVER_RATE).toFixed(1));
                ago_amt.push((user_data[key_value].LYTM_TURNOVER_RATE).toFixed(1));
        ...
        if(this.sel_value.day_set.day_7==true||this.sel_value.day_set.day_30==true){
          ...
          this.barData.month=((hour_data[hour_data.length-1].substring(0,2)=='12'&&year_add_time.length>1)?year_add_time[0]-1:year_add_time[0])+'.'+hour_data[hour_data.length-1];
          this.barData.value=this.fomatMoney(sale_amt[sale_amt.length-1])+
            (this.sel_value.target.all_muri_value==1||this.sel_value.target.all_muri_value==2?'m³'
            :this.sel_value.target.all_muri_value==3?'%'
            :this.sel_value.target.all_muri_value==4?'':'');

          this.barData.ago_value=this.fomatMoney(ago_amt[ago_amt.length-1])+
            (this.sel_value.target.all_muri_value==1||this.sel_value.target.all_muri_value==2?'m³'
            :this.sel_value.target.all_muri_value==3?'%'
            :this.sel_value.target.all_muri_value==4?'':'');
        ...
        this.chartIns2.setOption({
            ...
            tooltip: {
              ...
              formatter:function (params) {
                ...
                _this.barData.value =(params[1]==undefined)? 0:_this.fomatMoney(params[1].value)
                +(_this.sel_value.target.all_muri_value==1||_this.sel_value.target.all_muri_value==2?'m³'
                :_this.sel_value.target.all_muri_value==3?'%'
                :_this.sel_value.target.all_muri_value==4?'':'');

                _this.barData.ago_value =(params[0]==undefined)? 0: _this.fomatMoney(params[0].value)
                +(_this.sel_value.target.all_muri_value==1||_this.sel_value.target.all_muri_value==2?'m³'
                :_this.sel_value.target.all_muri_value==3?'%'
                :_this.sel_value.target.all_muri_value==4?'':'');
...
/deep/  .yoy_tb i.rate-up:before {
  right: 10px;
}

/deep/  .yoy_tb i.rate-down:before {
  right: 10px;
}
...
.ceshi1de1aaa::-webkit-scrollbar-thumb {//滚动条的设置
