  ...
  <div>
    <div class="table-box">
      <div class="top-content clearfix">
        <div class="title fl">B店推送报表(日报)</div>
...
export default {
  ...
  methods: {
    ...
    getTargetData() {
      this.$axios
        .post(`/api/admin/data/NON_PM_FS_YY_PROJECT_B/YJ_KPI/get`, {})
        .then((res) => {
        ...
        .then(() => {
          return this.$axios
            .post(`/api/admin/data/NON_PM_FF_YY_PROJECT_B/FLOW_D/get`, {})
            .then((res) => {
              console.log("==========最近7天=============");
              ...
              Object.keys(set_data).forEach((UserValue) => {
                ...
                month_day.push(set_data[UserValue].TDATE.substring(5, 10));
                all_visitors.push(set_data[UserValue].ALL_VISITORS.toFixed(2));
                ago_all_visitors.push(
                  set_data[UserValue].AGO_ALL_VISITORS.toFixed(2)
                );
                all_visitors_per_day.push(
                  set_data[UserValue].ALL_VISITORS_PER_DAY.toFixed(2)
                );

                pay_flow.push(set_data[UserValue].PAY_FLOW.toFixed(2));
                ago_pay_flow.push(set_data[UserValue].AGO_PAY_FLOW.toFixed(2));
                pay_flow_per_day.push(
                  set_data[UserValue].PAY_FLOW_PER_DAY.toFixed(2)
                );

                st_search.push(set_data[UserValue].ST_SEARCH.toFixed(2));
                ago_st_search.push(
                  set_data[UserValue].AGO_ST_SEARCH.toFixed(2)
                );
                st_search_per_day.push(
                  set_data[UserValue].ST_SEARCH_PER_DAY.toFixed(2)
                );

                st_recommend.push(set_data[UserValue].ST_RECOMMEND.toFixed(2));
                ago_st_recommend.push(
                  set_data[UserValue].AGO_ST_RECOMMEND.toFixed(2)
                );
                st_recommend_per_day.push(
                  set_data[UserValue].ST_RECOMMEND_PER_DAY.toFixed(2)
                );

                items_visitors.push(
                  set_data[UserValue].ITEMS_VISITORS.toFixed(2)
                );
                ago_items_visitors.push(
                  set_data[UserValue].AGO_ITEMS_VISITORS.toFixed(2)
                );
                items_visitors_per_day.push(
                  set_data[UserValue].ITEMS_VISITORS_PER_DAY.toFixed(2)
                );

                visitors_rate.push(
                  set_data[UserValue].VISITORS_RATE.toFixed(2)
                );
                ago_visitors_rate.push(
                  set_data[UserValue].AGO_VISITORS_RATE.toFixed(2)
                );
                visitors_rate_per_day.push(
                  set_data[UserValue].VISITORS_RATE_PER_DAY.toFixed(2)
                );

                items_pur_number.push(
                  set_data[UserValue].ITEMS_PUR_NUMBER.toFixed(2)
                );
                ago_items_pur_number.push(
                  set_data[UserValue].AGO_ITEMS_PUR_NUMBER.toFixed(2)
                );
                items_pur_number_per_day.push(
                  set_data[UserValue].ITEMS_PUR_NUMBER_PER_DAY.toFixed(2)
                );
                ...
                re_amt.push(set_data[UserValue].RE_AMT.toFixed(2));
                ago_re_amt.push(set_data[UserValue].AGO_RE_AMT.toFixed(2));
                re_amt_per_day.push(
                  set_data[UserValue].RE_AMT_PER_DAY.toFixed(2)
                );
              });
    ...
    getDetailData(date) {
      this.$axios
        .get("api/admin/biProjectBReport/getDetail", {
          params: { dateStr: date },
        })
        .then((res) => {
          this.dataMsg = [
        ...
        .then(() => {
          //获取净业绩图表数据
          return this.$axios
            .get("api/admin/biProjectBReport/getList1")
            .then((res) => {
              let timeDate = [];
              let numData = [];
        ...
        .then(() => {
          //获取未发取消图表数据
          return this.$axios
            .get("api/admin/biProjectBReport/getList2")
            .then((res) => {
              let timeDate = [];
              let numData = [];
              for (let i = res.data.length - 1; i >= 0; i--) {
        ...
        .then(() => {
          //获取客单价图表数据
          return this.$axios
            .get("api/admin/biProjectBReport/getList3")
            .then((res) => {
              let timeDate = [];
              let numData = [];
              for (let i = res.data.length - 1; i >= 0; i--) {
    ...
    initEchartsOet(SetName, Data, DataAgo, DataAvg, MonthDay, LegendName,Unit) {
      ...
      // if(Unit=='%'){
      //   this.chartOptions.tooltip.formatter= function (params) {
      //     return  params[0].axisValue+'<br/>'+params[0].marker+params[0].seriesName+'：'+params[0].value+'%<br/>'
